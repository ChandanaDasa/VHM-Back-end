...
const Vehicle = require('./vehicleRegister.js')
...
app.get('/id'/chassisNumber, async (req, res) => {
    ...
    const fields = 'name vehicleType vehicleModel email';
    ...
    if(!display){
        ...
        res.json({msg: "successfully name found...!!!", data:display})
    }
})
...
app.post('/login', async(req, res)=>{
    try{
        ...
        if(user.password === password) {
            ...
            res.json({message: 'Login successful', data:{name:user, email:user.emai}});
            }else{
                return res.status(401).json({message: 'Invalid email or password'});    
            }       
    } catch(err){
